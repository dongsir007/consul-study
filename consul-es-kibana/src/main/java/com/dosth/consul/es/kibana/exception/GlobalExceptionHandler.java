...
public class GlobalExceptionHandler {
	...
	public ResponseResult<String> bindExceptionHandler(IllegalArgumentException ex) {
		...
		log.warn("无效的请求参数 message={}", message, ex);
		return ResponseResult.badRequestError(message);
	}
	
	@ExceptionHandler(Throwable.class)
	@ResponseStatus(code = HttpStatus.OK)
	public ResponseResult<String> handlerApiException(Throwable exception, HttpServletRequest request) {
		...
		log.error("异常处理器url:{},message={},exception={}", url, exception.getMessage(), exception);
